...
export const listRecentTransactions = async (
  ...
  const { actions } = toListOptions(opts)
  ...
  const fallbackAggregate = async (): Promise<readonly Transaction[]> => {
    ...
    for (const id of userDocIds) {
      try {
        const rows = await listAllTransactionsForDocument(client, id, {})
    ...
    const byMonth = groupByMonth(filterTransactions(collected))
...
export const listTransactionsDatasetPage = async (
  ...
  try {
    ...
    return { items: fallback, nextOffset: null, usedFallback: false }
  } catch (error) {
    console.warn(
      `Fallback for dataset page failed: ${error instanceof Error ? error.message : String(error)}`
    )
    return { items: [], nextOffset: null, usedFallback: false }
  }
}
...
export const listAllTransactions = async (
  ...
  const sorted = sortDescByTimestamp(collected)
    ...
    try {
      rows = await listTransactionsForDocument(
        ...
        id,
        {
          limit: perPage,
          offset,
          actions: toListOptions(opts).actions,
        },
        signal
      )
    } catch (error) {
