...
import unleashData from './data/unleash-disable-spraakvelger.json' assert { type: 'json' }
...
export const getHandlers = (baseUrl: string = PATH) => [
  ...
  rest.get(`${baseUrl}/unleash`, (req, res, ctx) => {
    return res(ctx.status(200), ctx.json(unleashData), ctx.delay(10))
  }),
